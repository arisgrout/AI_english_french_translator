...
def _test_model(model, input_shape, output_sequence_length, french_vocab_size):
    ...
    print(model.compiled_loss._losses)
    assert len(model.compiled_loss._losses) > 0, "No loss function set.  Apply the `compile` function to the model."

    assert (
        "sparse_categorical_crossentropy" in model.compiled_loss._losses
...
def test_pad(pad):
    ...
    true_padded_tokens = np.array(
        [[i for i in range(4)] + [padding_id] * 2, [i for i in range(6)], [i for i in range(3)] + [padding_id] * 3]
    )
    assert isinstance(
