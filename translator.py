...
# %%
en = pd.Series(load_data("./data/small_vocab_en.txt")) 
#pd.Series(load_data("./data/europarl-v7.fr-en.en"))
fr = pd.Series(load_data("./data/small_vocab_fr.txt")) 
#pd.Series(load_data("./data/europarl-v7.fr-en.fr"))
...
nlp.add_pipe(nlp.create_pipe("sentencizer"))  # tokenization and sentence boundary detection

df_preproc = cleaner(df)


df_preproc['en_preproc'] = preprocess_parallel(df_preproc, nlp, 'en_clean', chunksize=1000)
df_preproc['fr_preproc'] = preprocess_parallel(df_preproc, nlp, 'fr_clean', chunksize=1000)
...
df_preproc['preproc'] = df_preproc['en_clean'].apply(lambda x: lemmatize(nlp, stopwords, x))
df_preproc['preproc'] = df_preproc['fr_clean'].apply(lambda x: lemmatize(nlp, stopwords, x))


print(stopwords)

# split into words (tokenize) - EXCLUDED FROM END ATM
...
def preprocess(text):
    ...
    text = rm_nonalpha(text)
    return text

# plug together & save steps
#df['en_clean'] = df.en_clean.apply(lambda x: preprocess(x))
#df['fr_clean'] = df.fr_clean.apply(lambda x: preprocess(x))
df['en_clean_lema'] = df.en_clean.apply(lambda x: lema(x))
df['fr_clean_lema'] = df.fr_clean.apply(lambda x: lema(x))
df['en_clean_stem'] = df.en_clean_lema.apply(lambda x: stem(x))
df['fr_clean_stem'] = df.fr_clean_lema.apply(lambda x: stem(x))
